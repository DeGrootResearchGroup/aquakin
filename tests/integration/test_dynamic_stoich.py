...
fixture model whose substrate coefficient is the symbolic expression
``-1/Y``.
"""
...
FIXTURE = Path(__file__).parents[1] / "fixtures" / "dynamic_stoich_model.yaml"
...
def test_summary_includes_symbolic_coefficient(model):
...
def test_dynamic_entries_recorded(model):
    """Compile registers the symbolic stoich entry."""
    assert len(model.stoich_dynamic) == 1
    i, j, _ = model.stoich_dynamic[0]
    assert i == 0  # only one reaction
    assert j == model.species_index["S"]


def test_compute_stoich_at_defaults(model):
    """compute_stoich(params) evaluates ``-1/Y`` at Y=0.5 -> -2.0."""
    stoich = model.compute_stoich(model.default_parameters())
    assert float(stoich[0, model.species_index["S"]]) == pytest.approx(-2.0)
    assert float(stoich[0, model.species_index["X"]]) == 1.0


def test_compute_stoich_responds_to_params(model):
    """Changing Y changes the substrate coefficient."""
    p = model.parameter_values({"Y": 0.25})
    stoich = model.compute_stoich(p)
    # -1/0.25 = -4
    assert float(stoich[0, model.species_index["S"]]) == pytest.approx(-4.0)


def test_dCdt_uses_dynamic_stoich(model):
    ...
    dC = model.dCdt(C, model.default_parameters(), conditions.fields, 0)
    assert float(dC[model.species_index["S"]]) == pytest.approx(-10000.0)
    assert float(dC[model.species_index["X"]]) == pytest.approx(5000.0)

    # Halving Y to 0.25 should double the substrate-side magnitude.
    p = model.parameter_values({"Y": 0.25})
    dC2 = model.dCdt(C, p, conditions.fields, 0)
    assert float(dC2[model.species_index["S"]]) == pytest.approx(-20000.0)
    assert float(dC2[model.species_index["X"]]) == pytest.approx(5000.0)


def test_ad_grad_through_stoich_only_param(model):
    ...
    g = jax.grad(loss)(model.default_parameters())
    Y_idx = model.param_index["Y"]
...
def test_static_path_unaffected():
    """Models with purely numeric stoich keep the empty-dynamic path."""
    # ozone_bromate, uv_h2o2, and the SUMO-derived ASM2/3 models all keep
...
def test_calibration_recovers_yield(model):
    """Generate noisy synthetic obs at a known Y, then fit Y back."""
    reactor = aquakin.BatchReactor(model, model.default_conditions(), rtol=1e-8, atol=1e-10)
    C0 = jnp.asarray([100.0, 50.0])

    # Truth: Y = 0.3, mu = 1.0 (default).
    true_params = model.parameter_values({"Y": 0.3})
...
"""
    ...
    with pytest.raises(ValueError, match="Stoichiometric coefficient"):
        aquakin.load_model_from_file(bad)
