"""Integration tests for the expanded ozone/bromate model."""
...
@pytest.fixture
def model():
    return aquakin.load_model("ozone_bromate")
...
def _run(model, *, OH_scavenging, t_end=600.0, n=61):
...
def test_shape_matches_v2(model):
    """Model shape after expansion."""
    assert model.n_species == 6
    assert model.n_reactions == 7
    assert "OH" in model.species
    assert "OH_scavenging" in model.conditions_required


def test_increasing_scavenging_reduces_bromate(model):
    """Higher matrix OH-scavenging should lower the OH-driven BrO3- yield."""
    sol_low = _run(model, OH_scavenging=1.0e4)
    sol_high = _run(model, OH_scavenging=1.0e6)
...
def test_OH_stays_in_radical_band(model):
    """OH should sit between 1e-15 and 1e-10 M throughout."""
    sol = _run(model, OH_scavenging=5.0e4)
    oh = sol.C_named("OH")
...
def test_bromate_bounded_by_bromide_inventory(model):
    """Final BrO3- cannot exceed initial Br-."""
    sol = _run(model, OH_scavenging=1.0e3)
