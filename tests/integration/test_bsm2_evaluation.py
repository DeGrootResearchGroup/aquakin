...
def test_aeration_energy_matches_fixed_kla(evaluated, asm1, adm1):
    ...
    _, _, _, ev = evaluated
    expected = (
        BSM2_DO_SATURATION / 1800.0 * sum(BSM2_TANK_VOLUMES[i] * BSM2_KLA[i] for i in range(5))
    )
...
def test_mixing_energy_counts_anoxic_reactors_and_digester(evaluated):
    ...
    from aquakin.plant.bsm.bsm2 import BSM2_DIGESTER_VOLUME, BSM2_TANK_VOLUMES
...
def test_carbon_mass_matches_dose(evaluated):
    ...
    from aquakin.plant.bsm.bsm2 import BSM2_CARBON_CONC, BSM2_CARBON_FLOW

    _, _, _, ev = evaluated
    assert ev.carbon_mass == pytest.approx(BSM2_CARBON_FLOW * BSM2_CARBON_CONC / 1000.0, rel=1e-6)
...
def test_pumping_energy_bsm2_kernel():
    ...
    t = jnp.array([0.0, 1.0])
    flows = {
        "internal": jnp.array([100.0, 100.0]),
...
def test_oci_bsm2_methane_offsets_heating():
    ...
    oci = operational_cost_index_bsm2(
        aeration=10, pumping=5, mixing=3, sludge_production=2, carbon=1, methane=50, heating=100
    )
...
def test_time_averaged_kernels_single_point_dont_crash():
    ...
    t1 = jnp.array([3.0])
    assert pumping_energy(
        t1, jnp.array([100.0]), jnp.array([50.0]), jnp.array([10.0])
    ) == pytest.approx(0.004 * 100 + 0.008 * 50 + 0.05 * 10, rel=1e-9)
    assert pumping_energy_bsm2(t1, {"internal": jnp.array([100.0])}) == pytest.approx(
        0.004 * 100, rel=1e-9
    )
    assert carbon_mass(t1, jnp.array([2.0]), carbon_conc=400000.0) == pytest.approx(
        2.0 * 400000.0 / 1000.0, rel=1e-9
    )  # 800 kg COD/d
    he = heating_energy(t1, jnp.array([100.0]), T_feed_C=15.0, T_target_C=35.0)
    assert he == pytest.approx(24.0 * 20.0 * 100.0 * 1000.0 * 4.186 / 86400.0, rel=1e-9)
    # One reactor unaerated, one aerated, at a single instant.
    me = mixing_energy(
        t1, jnp.array([[0.0, 100.0]]), jnp.array([1500.0, 3000.0]), digester_volume=3400.0
    )
...
def test_eqi_single_point_is_instantaneous(asm1):
    ...
    eqi2 = effluent_quality_index(jnp.array([4.0, 5.0]), jnp.tile(C, (2, 1)), jnp.tile(Q, 2), asm1)
...
def test_effluent_averages_single_point(asm1):
    ...
    C0 = asm1.concentrations({"XS": 10.0, "SS": 5.0, "SNO": 8.0, "SNH": 2.0})
    avg = effluent_averages(jnp.array([4.0]), C0[None, :], jnp.array([18000.0]), asm1)
...
def test_bsm2_report_is_labeled_with_units_and_breakdown():
    ...
    ev = BSM2Evaluation(
        eqi=6123.4,
        oci=0.0,
        aeration_energy=3784.2,
        pumping_energy=1689.0,
        mixing_energy=768.0,
        sludge_production=2280.5,
        carbon_mass=800.0,
        methane_production=1010.3,
        heating_energy=4200.0,
        effluent=_EFF,
...
def test_bsm1_report_is_labeled_and_str_delegates():
    ...
    ev = BSM1Evaluation(
        eqi=6443.2,
        oci=3341.4 + 388.2 + 240.0 + 5.0 * 2082.3,
        aeration_energy=3341.4,
        pumping_energy=388.2,
        sludge_production=2082.3,
        mixing_energy=240.0,
        effluent=_EFF,
        aerated_tanks=["tank3", "tank4", "tank5"],
    )
    ...
    assert ev.oci == pytest.approx(
        ev.aeration_energy + ev.pumping_energy + ev.mixing_energy + 5.0 * ev.sludge_production
    )


# ----- Report / kpi branches reachable without a solve --------------------


def test_bsm2_report_shows_blower_label_and_airflow():
    """The `air_flow is not None` (blower aeration) report branch."""
    from aquakin.plant.bsm import BSM2Evaluation
    ...
    r = ev.report()
    assert "blower" in r
    assert "air flow" in r


def test_bsm1_report_shows_blower_label_and_airflow():
    from aquakin.plant.bsm import BSM1Evaluation
...
def test_evaluation_kpis_direct_and_nan_effluent_fallback():
    """`kpis()` / `total_energy()` on a directly-constructed evaluation, with the
    empty-effluent NaN fallback (no solve)."""
    import math
    ...
    ev = BSM2Evaluation(
        ...
        pumping_energy=4.0,
        mixing_energy=5.0,
        sludge_production=6.0,
        carbon_mass=7.0,
        methane_production=8.0,
        heating_energy=9.0,
    )
    assert ev.total_energy() == pytest.approx(3.0 + 4.0 + 5.0)
    k = ev.kpis()
    energy = [v for key, v in k.items() if "Energy" in key]
    assert energy and energy[0] == pytest.approx(12.0)
    # effluent defaulted to {} -> the effluent-derived KPIs fall back to NaN.
    assert any(_isnan(v) for v in k.values())


def test_digester_gas_requires_adm1_digester():
    """`plant.digester_gas` raises on a plant with no ADM1 digester -- reached
    before any solution is read, so no solve is needed."""
    from aquakin.plant.bsm import build_bsm1

    plant = build_bsm1()
    with pytest.raises(ValueError, match="digester"):
        plant.digester_gas(None)
...
def test_eqi_weights_are_copp_alex_standard(asm1):
    ...
    expected = 1.0e4 * (2.0 * tss + 1.0 * cod + 2.0 * bod + 30.0 * tkn + 10.0 * sno) / 1000.0
