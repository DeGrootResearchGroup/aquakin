...
def test_solver_override_keys_cache_by_class(asm1):
    ...
    plant.solve(t_span=(0.0, 1.0), t_eval=t_eval, y0=y0)                       # Kvaerno5
    n_after_default = len(plant._jit_cache)
    plant.solve(t_span=(0.0, 1.0), t_eval=t_eval, y0=y0, solver=diffrax.Kvaerno3())
...
def test_solver_override_grad_flows(asm1):
    ...
    def loss(scale):
        sol = plant.solve(t_span=(0.0, 1.0), t_eval=jnp.array([1.0]),
                          ...
                          solver=diffrax.Kvaerno3(), gradient="jax_adjoint")
