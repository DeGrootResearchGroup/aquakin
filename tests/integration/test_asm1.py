"""Integration tests for the ASM1 built-in model."""
...
def _build(model, *, t_end=1.0, **C0_overrides):
    conditions = model.default_conditions()
    reactor = aquakin.BatchReactor(model, conditions, rtol=1e-6, atol=1e-9)
    C0 = model.concentrations(C0_overrides)
    sol = reactor.solve(
        C0,
        t_span=(0.0, t_end),
...
def test_shape(model):
    assert model.n_species == 13
    assert model.n_reactions == 8
    assert set(model.species) == {
        "SI", "SS", "XI", "XS", "XB_H", "XB_A", "XP",
        "SO", "SNO", "SNH", "SND", "XND", "SALK",
    }


def test_default_simulation_runs(model):
    """Default initial state should integrate without error over 1 day."""
    sol = _build(model)
    assert jnp.all(jnp.isfinite(sol.C))


def test_aerobic_substrate_consumed(model):
    ...
    sol = _build(model, t_end=0.01, SO=4.0, SS=80.0, XB_H=600.0)
    ss = sol.C_named("SS")
...
def test_nitrification_oxidises_ammonia(model):
...
def test_anoxic_denitrification(model):
...
def test_decay_only_no_substrate(model):
...
def test_AD_grad_through_solve(model):
...
def test_transforms_default_to_positive_log(model):
    """Most ASM1 rate constants are declared with positive_log transforms."""
    transforms = model.parameter_transforms
...
def test_yield_is_calibratable(model):
    ...
    g = jax.grad(loss)(model.default_parameters())
    Y_H_idx = model.param_index["Y_H"]
    assert jnp.all(jnp.isfinite(g))
    assert float(g[Y_H_idx]) != 0.0


def test_kinetic_params_are_shared_not_duplicated(model):
