...
network, the five solver settings, and ``solve`` / ``conditions``), so the
...
def test_batch_is_a_conditioned_reactor(simple_network):
    r = aquakin.BatchReactor(simple_network, _conds())
    assert isinstance(r, Reactor)
    assert isinstance(r, ConditionedReactor)


def test_pfr_is_a_conditioned_reactor(simple_network):
    r = aquakin.PlugFlowReactor(simple_network, _conds(), n_points=3,
                                length=1.0, velocity=1.0)
    assert isinstance(r, ConditionedReactor)


def test_biofilm_is_a_conditioned_reactor(simple_network):
    r = aquakin.BiofilmReactor(
        simple_network, _conds(), n_layers=2, thickness=1e-3,
        area_per_volume=50.0, diffusivity=1e-4, boundary_layer=1e-4,
    )
    assert isinstance(r, ConditionedReactor)


def test_particle_reactor_is_a_reactor_but_not_conditioned(simple_network):
