...
def test_check_gradient_finite_returns_finite_and_raises_on_nan(simple_network):
...
def test_check_gradient_finite_remedy_depends_on_dtmax(simple_network):
    ...
    uncapped = aquakin.BatchReactor(simple_network, cond)          # dtmax=None
...
def test_check_gradient_finite_guards_a_real_gradient(simple_network):
    ...
    g = reactor.check_gradient_finite(
        jax.grad(loss)(simple_network.default_parameters()), what="my gradient"
    )
