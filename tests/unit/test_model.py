...
def test_stoich_matrix_shape(simple_model):
    assert simple_model.stoich_matrix.shape == (1, 2)
    # A -> B with stoich A:-1, B:+1
    a_idx = simple_model.species_index["A"]
    b_idx = simple_model.species_index["B"]
    assert float(simple_model.stoich_matrix[0, a_idx]) == -1.0
    assert float(simple_model.stoich_matrix[0, b_idx]) == 1.0


def test_default_arrays(simple_model):
    C0 = simple_model.default_concentrations()
    assert C0.shape == (2,)
    p0 = simple_model.default_parameters()
    assert p0.shape == (1,)
    assert float(p0[0]) == pytest.approx(0.1)


def test_namespaced_param_keys(simple_model):
    assert "A_to_B.k" in simple_model.param_index
    assert simple_model.parameters == ["A_to_B.k"]


def test_rates_signature(simple_model):
    C = jnp.asarray([1.0, 0.0])
    params = simple_model.default_parameters()
    conditions = aquakin.SpatialConditions.uniform(1, T=293.15)
    r = simple_model.rates(C, params, conditions.fields, 0)
    assert r.shape == (1,)
    assert float(r[0]) == pytest.approx(0.1)


def test_dCdt(simple_model):
    C = jnp.asarray([1.0, 0.0])
    params = simple_model.default_parameters()
    conditions = aquakin.SpatialConditions.uniform(1, T=293.15)
    rhs = simple_model.dCdt(C, params, conditions.fields, 0)
    # dA/dt = -0.1, dB/dt = +0.1
    assert float(rhs[0]) == pytest.approx(-0.1)
    assert float(rhs[1]) == pytest.approx(0.1)


def test_summary_smoke(simple_model):
    out = simple_model.summary()
    assert "simple_decay" in out
    assert "A_to_B" in out


def test_species_units_and_descriptions(simple_model):
    """Species units/descriptions are carried from the YAML through compile."""
    assert simple_model.species_units == {"A": "mol/L", "B": "mol/L"}
    assert simple_model.species_descriptions == {"A": "Reactant", "B": "Product"}

    assert simple_model.units_of("A") == "mol/L"
    assert simple_model.description_of("B") == "Product"

    with pytest.raises(KeyError):
        simple_model.units_of("nope")
    with pytest.raises(KeyError):
        simple_model.description_of("nope")


def test_summary_includes_units(simple_model):
    out = simple_model.summary()
...
def test_time_unit_seconds(simple_model):
    """The fixture's rate constant is ``s-1``, so the model is in seconds."""
    assert simple_model.time_unit == "s"


def test_summary_includes_time_unit(simple_model):
    out = simple_model.summary()
...
def test_final_named_and_C_named_many_on_solution(simple_model):
    ...
    assert set(sol.final) == set(simple_model.species)
...
def test_solve_time_unit_conversion_is_equivalent(tmp_path):
    """Solving a days model with t in hours gives the same physics, with the
...
def test_solve_time_unit_unknown_raises(simple_model):
    reactor = aquakin.BatchReactor(simple_model, simple_model.default_conditions())
    with pytest.raises(ValueError, match="Unknown time_unit"):
        reactor.solve(simple_model.default_concentrations(),
                      t_span=(0.0, 1.0), time_unit="fortnight")


def test_solve_time_unit_undeclared_model_raises(tmp_path):
    """If the model's own time unit can't be inferred, conversion can't be
...
def test_to_latex_smoke(simple_model):
    latex = simple_model.to_latex()
...
def test_concentrations_by_name(simple_model):
    ...
    base = simple_model.default_concentrations()
    a = simple_model.species_index["A"]
    b = simple_model.species_index["B"]

    # No overrides -> the defaults unchanged.
    assert jnp.array_equal(simple_model.concentrations(), base)

    # Dict override.
    c = simple_model.concentrations({"A": 5.0})
    assert float(c[a]) == 5.0 and float(c[b]) == float(base[b])

    # kwargs override (identifier-safe name) and dict+kwargs together.
    c2 = simple_model.concentrations({"A": 5.0}, B=2.0)
    assert float(c2[a]) == 5.0 and float(c2[b]) == 2.0


def test_concentrations_zero_base(simple_model):
    """base='zero' starts every species at 0, so only the named ones are set --
    the correct base for a feed composition."""
    a = simple_model.species_index["A"]
    b = simple_model.species_index["B"]

    z = simple_model.concentrations({"A": 5.0}, base="zero")
    assert float(z[a]) == 5.0
    assert float(z[b]) == 0.0          # not the YAML default

    # No overrides + zero base -> all zeros.
    assert jnp.array_equal(
        simple_model.concentrations(base="zero"),
        jnp.zeros_like(simple_model.default_concentrations()),
    )

    # base='defaults' is the (default) old behaviour.
    assert jnp.array_equal(
        simple_model.concentrations({"A": 5.0}, base="defaults"),
        simple_model.concentrations({"A": 5.0}),
    )


def test_concentrations_invalid_base(simple_model):
    with pytest.raises(ValueError, match="base must be"):
        simple_model.concentrations({"A": 5.0}, base="nope")


def test_parameter_values_by_name(simple_model):
    p = simple_model.parameter_values({"A_to_B.k": 0.7})
    assert float(p[simple_model.param_index["A_to_B.k"]]) == 0.7
    # default (no overrides) equals default_parameters
    assert jnp.array_equal(simple_model.parameter_values(),
                           simple_model.default_parameters())


def test_atol_by_name(simple_model):
    atol = simple_model.atol({"B": 1e-15}, default=1e-9)
    assert float(atol[simple_model.species_index["B"]]) == 1e-15
    assert float(atol[simple_model.species_index["A"]]) == 1e-9
    assert atol.shape == (simple_model.n_species,)


def test_override_vector_is_ad_clean(simple_model):
    ...
    def c_of_x(x):
        return simple_model.concentrations({"A": x})[a]

    assert float(jax.grad(c_of_x)(5.0)) == 1.0

    # vmap over an override builds a batch of vectors.
    batched = jax.vmap(lambda x: simple_model.concentrations({"A": x}))(
        jnp.array([1.0, 2.0, 3.0])
    )
    assert batched.shape == (3, simple_model.n_species)
    ...
    k_idx = simple_model.param_index["A_to_B.k"]
    g = jax.grad(lambda x: simple_model.parameter_values({"A_to_B.k": x})[k_idx])
    assert float(g(0.7)) == 1.0
    b = simple_model.species_index["B"]
    ga = jax.grad(lambda x: simple_model.atol({"B": x})[b])
    assert float(ga(1e-12)) == 1.0


def test_override_unknown_name_raises_with_hint(simple_model):
    with pytest.raises(KeyError, match="Unknown species 'AA'"):
        simple_model.concentrations({"AA": 1.0})
    with pytest.raises(KeyError, match="Unknown parameter"):
        simple_model.parameter_values({"A_to_B.kk": 1.0})


def test_override_rejects_non_dict_positional(simple_model):
    with pytest.raises(TypeError, match="must be a dict"):
        simple_model.concentrations(["A", 1.0])


def test_compile_stage_helpers():
    """The compile_model stage helpers behave as a pipeline: parameter index
    is model-level-then-reaction-local in order, and _unresolved_params flags
    only names resolving to neither scope."""
    from aquakin.core.model import _build_param_index, _unresolved_params
    from aquakin.core.parser import parse_rate_expression

    # ASM1 has both model-level (Y_H, ...) and reaction-local params.
    net = aquakin.load_model("asm1")
    # Model-level params come before reaction-local (which are dotted).
    ...
    # 'k' resolves model-level; 'r1.kf' resolves reaction-local; 'nope' doesn't.
