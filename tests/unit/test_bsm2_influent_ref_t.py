"""The influent-T / model-ref_T consistency guard in ``bsm2_constant_influent``.

The influent temperature and the model's Arrhenius reference ``ref_T`` are
...
20 °C ``load_model("asm1")`` cuts nitrification by ~40 %. The guard warns
...
def test_mismatched_model_warns(asm1):
    """14.86 °C inlet on the plain 20 °C model (ref_T 293.15) warns, naming both."""
    ...
    assert "293.15" in msg                            # the model ref_T


def test_benchmark_pairing_is_silent():
    """The faithful pairing (15 °C-referenced model + 14.86 °C inlet) does not warn."""
    net = bsm2_asm1_model()
...
def test_exact_reference_is_silent():
    """An inlet exactly at the model reference is trivially consistent."""
    net = bsm2_asm1_model()
    with warnings.catch_warnings():
        warnings.simplefilter("error")
