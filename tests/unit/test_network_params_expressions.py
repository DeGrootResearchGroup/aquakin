"""Tests for network-level shared parameters and named rate expressions."""
...
# ---------- Network-level shared parameters ----------


def test_network_level_parameter_used_by_reaction(tmp_path):
    p = _yaml(tmp_path, """
        network: {name: shared, version: "1.0"}
        species:
...
def test_network_param_shared_across_reactions(tmp_path):
    """The same k is used in two reactions; single parameter slot."""
    p = _yaml(tmp_path, """
        network: {name: shared2, version: "1.0"}
        species:
          - {name: A, default_concentration: 1.0}
          - {name: B, default_concentration: 0.0}
...
def test_expression_self_reference_rejected(tmp_path):
    p = _yaml(tmp_path, """
        network: {name: self_ref, version: "1.0"}
        species:
          - {name: A, default_concentration: 1.0}
        parameters:
          k: {value: 1.0}
...
def test_expression_name_collides_with_global_param_rejected(tmp_path):
    p = _yaml(tmp_path, """
        network: {name: collide1, version: "1.0"}
        species:
          - {name: A, default_concentration: 1.0}
        parameters:
          rho: {value: 1.0}
...
def test_expression_name_collides_with_local_param_rejected(tmp_path):
    p = _yaml(tmp_path, """
        network: {name: collide2, version: "1.0"}
        species:
          - {name: A, default_concentration: 1.0}
        expressions:
          rho: "1.0 * [A]"
...
def test_unknown_identifier_after_resolution_errors(tmp_path):
    p = _yaml(tmp_path, """
        network: {name: unknown, version: "1.0"}
        species:
          - {name: A, default_concentration: 1.0}
        reactions:
          - name: r1
