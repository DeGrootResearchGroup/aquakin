...
A network file declares species, conditions, and reactions. The top-level
keys are:
...
parameters:             # optional: network-level (shared) parameters
  <name>:
    value: <float>
    units: <str>                # optional
...
Most networks of practical interest declare a kinetic constant (e.g.
...
reaction-specific values. A name cannot appear in both a network-level
...
literal** or a **string expression** in the network's parameters. Numeric
...
(reaction-local or network-level), and the arithmetic / negation operators.
...
network-level parameters with symbolic stoichiometry. Calibrating `Y_H`
