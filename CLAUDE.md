...
coefficients that depend on yield / N-content / fraction parameters are **live
symbolic string expressions**, re-evaluated from the current parameter vector
every `solve()` (via `CompiledNetwork.compute_stoich`), so calibrating
`Y_H` / `i_XB` / `f_P` and the like propagates to every dependent coefficient —
matching GPS-X/BioWin/SUMO, which keep the Gujer matrix live-coupled. Only cells
with no parameter leaf are constant-folded to numeric literals. The SUMO
export's `units` field is internal sentinels (`"0"` / `"SmallNumber"` /
`"-BigNumber"`) for parameters and an unparseable dotted dialect (`g COD.m-3`)
for species, so real units are stamped by the post-processor
[`networks/_fix_sumo_units.py`](aquakin/networks/_fix_sumo_units.py) (run after
regenerating). It assigns each unit from the network's own structure —
half-saturation constants get the currency of the species their Monod term
limits (so `check_units` always matches), rate constants get `1/d` (the
second-order precipitation constant `kPRE` gets `m3/(g_TSS*d)`), species get the
parseable `g_<currency>/m3` form (oxygen → `g_O2/m3` per the `asm1` convention,
not SUMO's negative-COD), and the stoichiometry-only yields / composition /
charge constants get best-effort advisory units. After it, all four resolve
`network.time_unit` to `"d"` and are `check_units`-clean **except** the two
ASM2d-TUD biomass-normalised PP-storage rates (`qPP·XPAO²/XPP·…`), whose root is
...
- `Plant` — assembles units and connections, drives the monolithic
  ...
  - **Adaptive AD-safe recycle convergence (`recycle_tol`).** The fixed
    ...
    `rho=0.9`, 97% at `rho=0.99`). `Plant(..., recycle_tol=...)` (default `None`)
    replaces the fixed mop-up with an **adaptive solve to that relative
    tolerance**, mirroring the charge-balance pH solver
    ...
    **streams** — flow `Q`, concentration `C`, and (when carried) temperature `T`
    ...
    early on a low-gain plant, and is **off by default** — `recycle_tol=None`
    keeps the fixed-pass path, bit-identical to the historic behaviour (the
    validated BSM steady states are untouched). **Verified** (`tests/integration/
    ...
    affine `recycle_map` still supplies the warm-start seed.
