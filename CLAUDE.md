...
- `wats_sewer` — extended WATS sewer-process model (carbon/sulfur/nitrogen
  ...
  experiments (`examples/wats_sewer_batch_validation.py`,
  `examples/wats_sewer_calibration.py`): after parameterizing the saturation
  constants the published study calibrated (the reference C hardcoded them) and
  AD-calibrating the influential rates, sulfide/sulfate/nitrate match the
  measured data well; VFA is the weak point (as in the source study), and the
  calibration/validation batches favour slightly different sulfur kinetics.
  First network to use a **state-derived pH** (see *Speciation / state-derived
  pH* below). **Documented deviation from the original published model:** anoxic
  ...
  coefficient and is fixed from the literature, not calibrated. The calibration
  example fits only the **identifiable** rate constants under a **relative**
  loss; the fixed-vs-free split is justified by the Laplace posterior
  correlation (e.g. `k_12_no`/`K_NO_f` are a near-degenerate ratio, so `K_NO_f`
  is fixed) rather than by manual trial — the AD analogue of the source study's
...
│   ├── lagrangian_demo.py
│   ├── sensitivity_demo.py
│   ├── wats_sewer_batch_validation.py  # reproduce sewer nitrate-dosing batch tests
│   ├── wats_sewer_calibration.py       # AD-calibrate wats_sewer to batch data
│   └── data/
│       └── wats_sewer_batch_reference.csv  # measured + reference-model series
│
├── docs/
│   ├── index.md
│   ├── network_format.md
