...
class Plant:
    ...
    def solve_sensitivity(
        ...
        *,
        ...
        # the flow setpoints).
        if self._recycle._recycle_map_constant is None:
            self._recycle._check_recycle_map_constant(t0a, y0, params_full)
        if self._recycle._flow_map_constant is None:
            self._recycle._check_flow_map_constant(t0a, y0, params_full)
        states0 = self._split_state(y0)
        rmap = self._recycle._maybe_recycle_map(t0a, states0, params_full)
        fmap = self._recycle._maybe_flow_map(t0a, states0, params_full)
