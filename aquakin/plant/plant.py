...
class ParameterLayout:
    """Maps named slices of the flat plant parameter vector to model /
    plant-level concerns.

    For BSM1 every CSTR uses the same ASM1 model so we have *one*
    kinetic-parameter block.

    The layout is generated by :meth:`Plant._build_parameter_layout` from
    the model references on each unit. Users typically don't construct
    it directly.
    """

    model_param_blocks: dict[str, tuple[int, int]] = field(default_factory=dict)
    # (start_index, length) per kinetic model, keyed by model.name.
    # All units sharing a model see the same block.
...
class Plant:
    ...
    def add_unit(self, unit: Unit) -> None:
        ...
        the per-model parameter blocks.
        """
    ...
    def add_influent(
        ...
        *,
        ...
        translator : StateTranslator, optional
            ...
            when the influent and destination share a model.
        """
    ...
    def set_temperature(self, celsius: float, *, units: Optional[Iterable[str]] = None) -> "Plant":
        ...
        ``temperature_corrections`` -- at that temperature. (For a model whose
    ...
    def connect(
        ...
        *,
        ...
        translator : StateTranslator, optional
            ...
            share a model; required when they differ.
        initial_value : Stream, optional
            ...
            stream of the source model. Pass it only to override that with a
    ...
    def _default_translator(
        self, source_model, dest_unit, translator, src_label, dst_label
    ) -> StateTranslator:
        """Return ``translator`` if given, else the default for the endpoints.

        The default is :class:`IdentityTranslator` when the source and
        destination share a model; a cross-model connection with no
        explicit translator is an error.
        """
        if translator is not None:
            return translator
        target_model = self._unit_model(dest_unit)
        if source_model is target_model:
            return IdentityTranslator(target_model)
        # Two *different* model objects. Distinguish the common mistake -- two
        # separate instances of the same model (e.g. calling bsm2_asm1_model()
        # twice, so the plant and the influent carry different temperature
        # corrections / parameters) -- from a genuine cross-model connection.
        same_model = getattr(source_model, "name", object()) == getattr(
            target_model, "name", None
        ) and getattr(source_model, "species", object()) == getattr(target_model, "species", None)
        if same_model:
            raise ValueError(
                f"Connection {src_label} -> {dst_label}: the two endpoints use "
                f"different *instances* of the same model "
                f"('{getattr(source_model, 'name', '?')}'). Build the model "
        ...
        raise ValueError(
            f"Connection {src_label} -> {dst_label} crosses models "
            f"('{getattr(source_model, 'name', '?')}' -> "
            f"'{getattr(target_model, 'name', '?')}'); supply an explicit "
            f"translator."
        )
    ...
    def _finalize_topology(self) -> None:
        ...
        self._recycle_seeds = {}
        for c in auto_cut:
            net = self.units[c.from_unit].model
            self._recycle_seeds[(c.from_unit, c.from_port)] = Stream(
                Q=jnp.asarray(0.0), C=net.default_concentrations(), model=net
            )

    def _unit_model(self, unit_name: str) -> CompiledModel:
        unit = self.units[unit_name]
        if not hasattr(unit, "model"):
            raise KeyError(
                f"Unit '{unit_name}' has no 'model' attribute; cannot infer translator default."
            )
        return unit.model
    ...
    def _materialize_dosing(self) -> None:
        ...
        for cid, members in groups.items():
            ...
            self.add_unit(
                PIController(
                    name=cid,
                    model=self.units[d0.sensor].model,
    ...
    def _ordered_models(self) -> list[CompiledModel]:
        """The distinct kinetic models, in first-appearance order.

        Deduplicated by object **identity** (two units sharing the same
        compiled model see one block). Parameter blocks are keyed by
        ``model.name``, so two *distinct* models sharing a name would
        ...
        """
        seen: list[CompiledModel] = []
        for name in self._insertion_order:
            net = getattr(self.units[name], "model", None)
            if net is None:
                continue
            ...
            if any(s.name == net.name for s in seen):
                raise ValueError(
                    f"Two distinct models share the name '{net.name}'. "
                    f"Parameter blocks are keyed by model name, so names "
                    f"must be unique across the plant's models."
                )
            seen.append(net)
        return seen
    ...
    def time_unit(self) -> Optional[str]:
        ...
        kinetic model's rate constants (:attr:`CompiledModel.time_unit`) --
        ``"d"`` for the BSM-family plants (ASM1 + ADM1 are both in days).
        Returns ``None`` if the plant has no kinetic model, or its models
        disagree on (or do not declare) a time unit -- in which case a
        ``time_unit=`` conversion in :meth:`solve` cannot be applied.
        """
        units = {net.time_unit for net in self._ordered_models()}
        if len(units) == 1:
            return next(iter(units))
        return None

    def _build_parameter_layout(self) -> ParameterLayout:
        """Concatenate kinetic-parameter vectors for every distinct model.

        Each model contributes one block, in the order models first
        appear via the unit-list iteration.
        """
        blocks: dict[str, tuple[int, int]] = {}
        cursor = 0
        net_index: dict[str, int] = {}
        for i, net in enumerate(self._ordered_models()):
        ...
        self._model_param_index = net_index
        self._parameter_layout = ParameterLayout(
            model_param_blocks=blocks,
    ...
    def _kinetic_param_size(self) -> int:
        """Total length of the kinetic (model) parameter blocks."""
        layout = self._parameter_layout
        return sum(size for _, size in layout.model_param_blocks.values())

    def _coerce_params(self, params: jnp.ndarray) -> jnp.ndarray:
        """Accept a full parameter vector, or a kinetic-only one (padded).

        The full vector is ``[kinetic models | flow setpoints]``. A vector of
        only the kinetic length -- e.g. one built by concatenating the model
    ...
    def _plant_param_index(self) -> dict[str, int]:
        """Map ``"<model>.<param>"`` -> index in the flat plant parameter vector.

        Built from the per-model parameter blocks (:attr:`model_param_blocks`)
        and each model's own ``param_index``. The key prefixes a model's
        namespaced parameter name with the model name, so the same flat vector
        the integrator uses gets a friendly by-name address.
        """
        self._build_parameter_layout()
        blocks = self._parameter_layout.model_param_blocks
        index: dict[str, int] = {}
        for net in self._ordered_models():
    ...
    def parameter_names(self) -> list[str]:
        ...
        Each kinetic model contributes its (namespaced) parameter names
        prefixed by the model name -- e.g. ``"asm1.muH"`` or
    ...
    def parameter_index(self, name: str) -> int:
        ...
        *position* rather than a new vector -- e.g. ``jax.grad`` with respect to
        ...
        materialises concrete values) -- without hand-computing the model
        ...
        if name not in index:
            ...
            raise KeyError(
                ...
                f"'<model>.<param>' (see plant.parameter_names()).{suffix}"
            )
        return index[name]
    ...
    def parameter_values(self, overrides: Optional[dict] = None, /) -> jnp.ndarray:
        ...
        ``"<model>.<param>"`` -- the model name plus the model's own
        (namespaced) parameter name -- so you can bump one rate in a
        multi-model plant (e.g. BSM2's ASM1 water line + ADM1 digester)
        without hunting the block offset and index by hand.

        Parameters
        ----------
        overrides : dict[str, float], optional
            Map of ``"<model>.<param>"`` to a new value. Unknown names raise a
        ...
        if not isinstance(overrides, dict):
            raise TypeError(
                "overrides must be a dict of '<model>.<param>' -> value; got "
    ...
    def _is_concentration_unit(unit) -> bool:
        """Whether a unit's state *is* its model's concentration vector (so it
        ...
        return hasattr(unit, "model") and getattr(unit, "state_size", 0) == unit.model.n_species
    ...
    def list_species(self, unit: str) -> list[str]:
        ...
        return list(u.model.species)

    def _params_for_unit(self, unit_name: str, params_full: jnp.ndarray) -> jnp.ndarray:
        """Slice out one unit's parameters: its kinetic model block, then (for
    ...
    def _slice_unit_params(self, unit_name: str, params_full: jnp.ndarray) -> jnp.ndarray:
        ...
        if net is None:
            ...
            start, size = self._parameter_layout.model_param_blocks[net.name]
    ...
    def _resolve_streams(
        ...
        carries a non-JAX ``model`` field and so cannot be a differentiated
        leaf; the model is supplied here from the recorded influent.)
        """
    ...
    def _cached_streams(self, solution: "PlantSolution", params_full):
        ...
        **caches** the result on the solution keyed by the parameter vector --
        ...
        # only the (Q, C) arrays (the Stream's `model` is a static, non-JAX
    ...
    def stream(
        ...
        **The plant integrates unit states, not the inter-unit streams**, so a
        ...
        -------
        StreamSeries
            ``t``, ``Q``, ``C`` (shape ``(n_t, n_species)``) and ``model``,
        ...
        return StreamSeries(t=solution.t, Q=Q, C=C, model=self.units[unit].model, org=org)
    ...
    def mass_balance(
        ...
        *,
        ...
        so inventories and fluxes sum across models of different units (the ASM
        ...
        components : tuple of str, optional
            Which components to balance (default ``("COD", "N", "P")``); a model
        ...
        for name in self._unit_order:
            ...
            for conn in self._inputs_by_unit.get(name, ()):
                ...
                sensed_outputs[(conn.from_unit, conn.from_port)] = Stream(
                    ...
                    model=self.units[conn.from_unit].model,
                )
    ...
    def steady_state(
        ...
        **Differentiable.** The returned ``state`` carries the
        ...
        scale_floor : float or array, optional
            ...
            multi-model plant (the flat scalar floor over-damps small-magnitude
        ...
        colored_jacobian : bool
            ...
            on its sparsity-pattern support: bit-identical on a single-model
            plant, identical to PTC tolerance on a multi-model plant where the
        ...
        # a multi-model plant. The cached map equals the probed map, so the
        ...
        for spec in operating or []:
            ...
            if kind == "influent_flow":
                ...
                if sp not in net.species_index:
                    raise KeyError(
                        f"operating species {sp!r} is not in the influent model for port {port!r}."
                    )
