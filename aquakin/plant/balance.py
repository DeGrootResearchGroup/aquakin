...
def _unit_inventory(plant, unit_name, state_vec, content_by_network, params):
    ...
    # Layered Takács settler: state is (n_layers, n_part); inventory is the
    # particulate blanket summed over layers at the per-layer volume.
    if hasattr(unit, "_part_indices") and hasattr(unit, "n_layers"):
        layer_vol = float(unit.area) * float(unit.height) / int(unit.n_layers)
        prof = np.asarray(state_vec).reshape(int(unit.n_layers), unit._n_part)
        out = {}
