"""Pydantic models for YAML model files.

These models are used only at load time. Once a ``ModelSpec`` has been
validated, it is converted to a :class:`~aquakin.core.model.CompiledModel`
...
class ModelMeta(BaseModel):
    """Top-level model metadata (``model:`` block)."""
...
class SpeciesSpec(BaseModel):
    ...
    # the conservation check (:meth:`CompiledModel.check_conservation`) dots them
    # against the stoichiometry. Advisory metadata: declaring it lets a model
...
class TemperatureCorrectionSpec(BaseModel):
    ...
    (Kelvin for the ASM/ADM models), and a difference is used, so Kelvin and
...
class ReactionSpec(BaseModel):
    ...
    # the model default) instead of written by hand -- so a
...
class ModelSpec(BaseModel):
    ...
    def _check_consistency(self) -> "ModelSpec":
        ...
        if not self.reactions:
            raise ValueError(
                "model has no reactions: declare a 'reactions:' list, or a "
        ...
        if collide_globals:
            raise ValueError(
                f"Names appear in both model parameters and expressions: {sorted(collide_globals)}"
            )
        for rxn in self.reactions:
            ...
            if shadowed:
                raise ValueError(
                    ...
                    f"{sorted(shadowed)} that shadow model-level parameter(s). "
