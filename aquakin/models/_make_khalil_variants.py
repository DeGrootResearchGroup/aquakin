...
HALFORDER_RATES = {
    "sulfide_oxidation_anoxic_biofilm": "k_sII_anox_f * ho_sumS_half * ho_SNO_half * {A_V}",
    "elemental_S_oxidation_anoxic_biofilm": "k_s0_anox_f * ho_XS0_half * ho_SNO_half * {A_V}",
}
...
def apply_srbsubstrate(net):
    ...
    net["reactions"] = [
        r
        for r in net["reactions"]
        if r["name"] not in ("sulfate_reduction_VFA_biofilm", "elemental_S_reduction_VFA_biofilm")
    ]
    net["reactions"] += [
        {
...
def apply_stopatS0(net):
    ...
    net["reactions"] = [
        r for r in net["reactions"] if r["name"] != "elemental_S_oxidation_anoxic_biofilm"
    ]
...
VARIANT_DESC = {
    "halforder": (
        "Half-order (square-root) kinetics for the nitrate-driven biofilm "
        "sulfur-oxidation reactions, in place of the Monod terms."
    ),
    "directsulfate": (
        "Nitrate-driven sulfide oxidation proceeds directly to sulfate "
        "in one step, bypassing the elemental-sulfur intermediate."
    ),
    "srbsubstrate": (
        "Sulfate and elemental-sulfur reduction consume readily-"
        "biodegradable substrate (S_B) rather than VFA."
    ),
    "stopatS0": (
        "The nitrate-driven oxidation stops at elemental sulfur: the "
...
def _halforder_extends_doc(base_name):
    return {
        ...
        "reactions": [{"name": name, "rate": rate} for name, rate in HALFORDER_RATES.items()],
    }
...
def build(base_name, variant_keys, standalone_keys=()):
    ...
    for key in list(variant_keys) + ["combined"] + list(standalone_keys):
        ...
        header = (
            f"# Auto-generated from {base_name}.yaml by "
        ...
        net["model"]["name"] = f"{base_name}_{key}"
        ...
        fns = [VARIANT_FNS[k] for k in variant_keys] if key == "combined" else [VARIANT_FNS[key]]
        for fn in fns:
            fn(net)
        with open(out, "w") as f:
            f.write(header)
